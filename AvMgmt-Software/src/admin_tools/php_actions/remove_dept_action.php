<?php
	...
	//Database Queries
	$q1 = "select id from departments where id = $dept;";
	...
	$result1 = mysqli_query($link, $q1); //finds out if the department exists
	
	//Department Deletion Query
	$q2 = "delete from departments where id = $dept;";
	
	//Checks if the department they entered exists
	if(mysqli_num_rows($result1) != 0){ //if it doesn't exist
		if(!mysqli_query($link, $q2)){ //run the deletion query
		...
		echo("Error description: Department doesn't exist in the database.");
